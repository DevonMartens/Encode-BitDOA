...
describe("VolcanoCoin", () => {
  ...
  beforeEach(async () => {
    const VolcanoV1 = await ethers.getContractFactory("VolcanoCoinV1");
    volcanoContract = await upgrades.deployProxy(VolcanoV1, { kind: 'uups' });
  ...
  it("deploys and upgrades from V1 to V2 correctly", async () => {
    let version = await volcanoContract.version();
    expect(version).to.equal(1);

    const VolcanoCoinV2 = await ethers.getContractFactory("VolcanoCoinV2");
    volcanoContract = await upgrades.upgradeProxy(volcanoContract.address, VolcanoCoinV2);
    await volcanoContract.deployed();

    version = await volcanoContract.version();
